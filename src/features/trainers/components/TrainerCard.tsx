...
export function TrainerCard({ trainer }: Props) {
  return (
    <li className="group transition hover:-translate-y-0.5">
      <Card className={cn(styles.card, 'overflow-hidden h-full transition hover:border-zinc-300 hover:shadow-md')}>
        <CardImage className="w-full h-96 object-cover" src={`https://picsum.photos/350?random=${trainer.id}`} alt={trainer.name} />
